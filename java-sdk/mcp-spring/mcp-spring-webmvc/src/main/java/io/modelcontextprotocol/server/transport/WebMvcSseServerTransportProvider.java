...
public class WebMvcSseServerTransportProvider implements McpServerTransportProvider {
	...
	private ServerResponse handleSseConnection(ServerRequest request) {
		...
		logger.info("Creating new SSE connection for session: {}", sessionId);
	...
	private class WebMvcMcpSessionTransport implements McpServerTransport {
		...
		WebMvcMcpSessionTransport(String sessionId, SseBuilder sseBuilder) {
			...
			logger.info("Session transport {} initialized with SSE builder", sessionId);
		}
		...
		public Mono<Void> closeGracefully() {
			return Mono.fromRunnable(() -> {
				...
				try {
					sseBuilder.complete();
					logger.info("Successfully completed SSE builder for session {}", sessionId);
				}
				catch (Exception e) {
