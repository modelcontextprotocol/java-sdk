...
public class McpClientSession implements McpSession {
	...
	public McpClientSession(Duration requestTimeout, McpClientTransport transport,
		...
		// consumer
		this.connection = Mono.from(this.transport.connect(mono -> Mono.from(mono).doOnNext(this::handle))).subscribe();
	}
	 ...
	 * @return A Publisher that completes when the session is closed
	 */
	@Override
	public Mono<Void> closeGracefully() {
		this.connection.dispose();
		return Mono.from(transport.closeGracefully());
	}
