 ...
 * {@link ServerSessionFactory} which will be used to handle a 1:1 communication between a
 * newly connected client and the server. The provider's responsibility is to create
 * instances of {@link McpServerTransport} that the session will utilise during the
 * session lifetime.
 *
 * <p>
 ...
 * the {@link McpSession#close()} or {@link McpSession#closeGracefully()} closes the
 * provided transport.
 *
...
public interface McpServerTransportProvider {
	 ...
	 * @return a Publisher that completes when the notification has been broadcast
	 * @see McpSession#sendNotification(String, Object)
	 */
	Publisher<Void> notifyClients(String method, Object params);
	 ...
	 * @return a {@link Publisher<Void>} that completes when the connections have been
	 * closed.
	 */
