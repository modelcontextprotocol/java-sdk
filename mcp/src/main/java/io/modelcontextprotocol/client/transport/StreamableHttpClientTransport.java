 ...
 * follows the Streamable HTTP transport specification from protocol version 2025-03-26.
 *
 * <p>
...
public class StreamableHttpClientTransport implements McpClientTransport {
	...
	public static class Builder {
		...
		public Builder withRequestBuilder(HttpRequest.Builder requestBuilder) {
			...
			else if (!headers.get(ACCEPT_HEADER_NAME).containsAll(REQUIRED_ACCEPTED_CONTENT)) {
				logger.warn(
	...
	private void setupSseConnection() {
		...
		HttpRequest.Builder getBuilder = HttpRequest.newBuilder(getUri)
			.GET()
			.header(ACCEPT_HEADER_NAME, ACCEPT_HEADER_GET_VALUE);
	...
	public Mono<Void> sendMessage(JSONRPCMessage message) {
		...
		try {
			...
			if (!builder.build().headers().map().containsKey(ACCEPT_HEADER_NAME)) {
				builder.header(ACCEPT_HEADER_NAME, ACCEPT_HEADER_POST_VALUE);
			}
			if (!builder.build().headers().map().containsKey(CONTENT_TYPE_HEADER_NAME)) {
				builder.header(CONTENT_TYPE_HEADER_NAME, CONTENT_TYPE_HEADER_VALUE);
			}
