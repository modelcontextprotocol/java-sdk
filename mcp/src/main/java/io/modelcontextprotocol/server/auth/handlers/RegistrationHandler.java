...
public class RegistrationHandler {
	...
	public RegistrationHandler(OAuthAuthorizationServerProvider provider, ClientRegistrationOptions options) {
	...
	public CompletableFuture<OAuthClientInformation> handle(OAuthClientMetadata clientMetadata) {
		...
		if (clientMetadata.getRedirectUris() == null || clientMetadata.getRedirectUris().isEmpty()) {
			return CompletableFuture.failedFuture(
					new RegistrationException("invalid_redirect_uri", "At least one redirect URI is required"));
		}
		...
		for (URI redirectUri : clientMetadata.getRedirectUris()) {
			if (!isValidRedirectUri(redirectUri)) {
				return CompletableFuture.failedFuture(
						new RegistrationException("invalid_redirect_uri", "Invalid redirect URI: " + redirectUri));
			}
		}
			...
			for (String scope : requestedScopes) {
				if (!options.getValidScopes().contains(scope)) {
					return CompletableFuture
						.failedFuture(new RegistrationException("invalid_scope", "Invalid scope: " + scope));
				}
			}
		}
