...
public class RevocationHandler {
	...
	public RevocationHandler(OAuthAuthorizationServerProvider provider, ClientAuthenticator clientAuthenticator) {
	...
	public CompletableFuture<Void> handle(Map<String, String> params) {
		...
		return clientAuthenticator.authenticate(clientId, clientSecret).thenCompose(client -> {
			...
			if ("refresh_token".equals(tokenTypeHint)) {
				return provider.loadRefreshToken(client, token).thenCompose(refreshToken -> {
				...
				return provider.loadAccessToken(token).thenCompose(accessToken -> {
					if (accessToken != null && accessToken.getClientId().equals(client.getClientId())) {
